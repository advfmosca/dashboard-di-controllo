...
def per_client_op(a, idx_in_group=0, project=None, today_iso=None):
    ...
    if cause == "ad_fatigue":
        if freq >= 2.0:
            diagnosi = (f"il pubblico (donne) vede ormai troppe volte la stessa inserzione "
                        f"(esposizione media {freq:.2f} volte a persona)")
        elif hist_leads >= 8 and last_lead:
            diagnosi = f"aveva generato {hist_leads} contatti negli ultimi 14 giorni (ultimo il {_fmt_it_date(last_lead)}), poi si è fermato"
        elif hist_leads >= 2 and last_lead:
            diagnosi = f"i {hist_leads} contatti recenti (ultimo il {_fmt_it_date(last_lead)}) si sono fermati"
        else:
